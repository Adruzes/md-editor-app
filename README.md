...
このリポジトリには、性質の異なる2つの実行方式があります。

| | 軽量版（ブラウザ間借り方式） | Electron版（完全スタンドアロン） |
|---|---|---|
| ステータス | **✅ 動作確認済み・メインで使っているのはこちら** | 🚧 開発途中・未検証（ソースのみ、ビルド未確認） |
| 起動ファイル | `start_mdnote.exe` / `.bat` / `.command` | なし（[自分でビルド](#electron版のビルド方法)する必要があります） |
| サイズ | 約1.6MB | 約150〜360MB（見込み） |
| 起動の仕組み | PC に入っている Chrome か Edge を `--app` モードで裏から呼び出す | Chromium エンジンをアプリ自体に内蔵する想定。ブラウザに一切依存しない |
| メリット | 圧倒的に軽い・起動が速い・動作確認済み | （完成すれば）VSCode と同じ方式で、ブラウザの有無に左右されない単体アプリになる |
| デメリット | 内部的には Chrome/Edge を使っている（Windows なら標準搭載の Edge が使われることが多い） | サイズが大きくGitHubには載せていない／ビルド手順はあるが最終動作確認がまだ済んでいない |

**現状は軽量版のみが実用段階です。** Electron版はソースコード（`electron-app/`）だけこのリポジトリに含まれていますが、ビルド済みバイナリは大きすぎるため GitHub にはアップロードしていません。動作もまだ完全には確認できていない、開発中のものとして扱ってください。

## 使い方

### 起動

- **Windows**: `start_mdnote.exe` をダブルクリック（`start_mdnote.bat` でも同じ動作をします）
- **Mac**: `start_mdnote.command` をダブルクリック（初回は右クリック→「開く」が必要な場合があります）

アドレスバーやタブのない、独立したウィンドウとして起動します。

> どちらも署名なしの実行ファイルです。初回起動時に Windows の「WindowsによってPCが保護されました」という警告が出た場合は、「詳細情報」→「実行」で起動できます。

（Electron版は現在開発中のため、起動手順は[こちら](#electron版のビルド方法)のビルド手順のみです）
...
## Electron版のビルド方法（開発中・未検証）

`electron-app/` にソース一式が入っていますが、**まだ最終的な動作確認ができていない開発中の機能**です。ビルド済みのバイナリはサイズが大きいため GitHub には含めていません。試す場合は以下の手順で自分でビルドしてください。
